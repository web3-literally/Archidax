  ...
  module V1
    class Accounts < Grape::API
      ...
      resource :accounts do
        ...
        get '/me' do
          user_info = current_account.as_json(only: %i[uid email level role state otp_enabled])
          unless current_account.profile
            user_info
          else
            user_info.merge(current_account.profile.as_json(only: %i[first_name last_name]))
          end
        end
