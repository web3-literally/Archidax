...
namespace :seed do
  ...
  desc 'Adds missing otc_markets to database defined at config/seed/otc_markets.yml.'
  task otc_markets: :environment do
    OtcMarket.transaction do
      YAML.load_file(Rails.root.join('config/seed/otc_markets.yml')).each do |hash|
        next if OtcMarket.exists?(id: hash.fetch('id'))
        OtcMarket.create!(hash)
      end
    end
  end
