...
class ApplicationController < ActionController::Base
  ...
  helper_method :current_user, :is_admin?, :current_market, :current_otc_market, :gon
...
private
  ...
  def current_otc_market
    unless params[:otc_market].blank?
      OtcMarket.enabled.find_by_id(params[:otc_market])
    end || OtcMarket.enabled.ordered.first
  end
  ...
  def set_gon
    # zealousWang todo: need to add otc part here
    gon.environment = Rails.env
    gon.local = I18n.locale
    gon.market = current_market.attributes
    gon.otc_market = current_otc_market.attributes
    gon.ticker = current_market.ticker
    gon.otc_ticker = current_otc_market.otc_ticker
    gon.markets = Market.enabled.each_with_object({}) { |market, memo| memo[market.id] = market.as_json }
    gon.otc_markets = OtcMarket.enabled.each_with_object({}) { |otc_market, memo| memo[otc_market.id] = otc_market.as_json }
    ...
    gon.i18n = {
      ask: I18n.t('gon.ask'),
      bid: I18n.t('gon.bid'),
      otc_ask: I18n.t('gon.otc_ask'),
      otc_bid: I18n.t('gon.otc_bid'),
      cancel: I18n.t('actions.cancel'),
      accept: I18n.t('actions.accept'),
      reject: I18n.t('actions.reject'),
      latest_trade: I18n.t('private.markets.order_book.latest_trade'),
      latest_otc_trade: I18n.t('private.markets.order_book.latest_otc_trade'),
      switch: {
    ...
    # zealousWang todo : need to check this
    gon.otc_tickers = {}
    OtcMarket.enabled.each do |otc_market|
      gon.otc_tickers[otc_market.id] = otc_market.unit_info.merge(Global[otc_market.id].otc_ticker)
    end
