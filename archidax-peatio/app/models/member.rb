...
class Member < ActiveRecord::Base
    ...
    def from_auth(auth_hash)
      ...
      member.tap do |member|
        member.transaction do
          ...
          member.name     = "#{info_hash['first_name']} #{info_hash['last_name']}" if info_hash.key?('first_name')
          member.save!
  ...
  def otc_trigger_pusher_event(event, data)
    self.class.otc_trigger_pusher_event(self, event, data)
  end
...
private
  ...
  class << self
    ...
    def otc_trigger_pusher_event(member_or_id, event, data)
      AMQPQueue.enqueue :otc_pusher_member, \
        member_id: self === member_or_id ? member_or_id.id : member_or_id,
                        event:     event,
                        data:      data
    end
