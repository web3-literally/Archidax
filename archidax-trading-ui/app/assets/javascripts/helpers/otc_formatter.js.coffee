class OtcFormatter
  ...
  fix: (type, str) ->
    ...
    if type is 'ask'
      @.round(str, gon.otc_market.ask_precision)
    else if type is 'bid'
      @.round(str, gon.otc_market.bid_precision)
  ...
  fixPriceGroup: (str) ->
    if gon.market.price_group_fixed
      str = '0' unless $.isNumeric(str)
      @.round(str, gon.otc_market.price_group_fixed)
    else
      @fixBid(str)
  ...
  otc_market: (base, quote) ->
    "#{base.toUpperCase()}/#{quote.toUpperCase()}"

  otc_market_url: (otc_market, otc_order_id) ->
    if otc_order_id?
      "/otc_markets/#{otc_market}/otc_orders/#{otc_order_id}"
    else
      "/otc-trading/#{otc_market}"

  otc_trade: (ask_or_bid) ->
    gon.i18n[ask_or_bid]

  otc_short_trade: (type) ->
...
window.otc_formatter = new OtcFormatter()
